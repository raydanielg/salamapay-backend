...
class RemoveAdminPageCommand extends Command
{
    /**
     * The name and signature of the console command.
     */
    protected $signature = 'tyro-dashboard:remove-admin-page
                            {name? : The name of the page to remove}';

    /**
     * The console command description.
     */
    protected $description = 'Remove an admin dashboard page';
    ...
    public function handle(): int
    {
        ...
        if (!$name) {
            $name = $this->ask('What is the name of the admin page to remove?');
        ...
        $this->info('');
        $this->info('  ╔════════════════════════════════════════╗');
        $this->info('  ║   Removing Admin Dashboard Page        ║');
        $this->info('  ╚════════════════════════════════════════╝');
        $this->info('');
        ...
        $this->warn('     • Sidebar link from admin-sidebar.blade.php');
        $this->info('');
    ...
    protected function removeRouteFromWebFile(string $pageName): void
    {
        ...
        // Pattern to match the route line (with admin middleware)
        $patterns = [
            "/Route::view\('dashboard\/{$pageName}', 'dashboard\.{$pageName}'\)->middleware\(\['auth', 'tyro-dashboard\.admin'\]\)->name\('dashboard\.{$pageName}'\);?\n?/",
            "/Route::view\(\"dashboard\/{$pageName}\", \"dashboard\.{$pageName}\"\)->middleware\(\['auth', 'tyro-dashboard\.admin'\]\)->name\('dashboard\.{$pageName}'\);?\n?/",
        ];
