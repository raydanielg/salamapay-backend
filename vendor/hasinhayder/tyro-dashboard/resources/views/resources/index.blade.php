                ...
                <tr style="cursor: pointer;" onclick="window.location='{{ route('tyro-dashboard.resources.show', [$resource, $item->id]) }}'">
                    ...
                    <td style="text-align: right;" onclick="event.stopPropagation()">
                        <div class="table-actions" style="justify-content: flex-end;">
                            ...
                            <form action="{{ route('tyro-dashboard.resources.destroy', [$resource, $item->id]) }}" method="POST" style="display: inline;" id="delete-resource-item-form-{{ $item->id }}">
                                @csrf
                                @method('DELETE')
                                <button type="button" class="btn btn-icon btn-ghost text-danger" title="Delete" onclick="event.preventDefault(); showDanger('Delete Item', 'Are you sure you want to delete this item?').then(confirmed => { if(confirmed) document.getElementById('delete-resource-item-form-{{ $item->id }}').submit(); })">
                                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
